...
import { bookTreatmentUrl } from '../utils/bookUrl';
import {
  treatmentIcon,
  bodyAreaStyle,
  parsePlanPhases,
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-lg mx-auto px-4 py-20 text-center flex-1">
          ...
          <Link to="/treatments" className="btn-primary inline-block mt-6">
            View all treatments
          </Link>
        </div>
        <Footer />
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <section className="bg-gradient-to-br from-orange-50 via-white to-primary-50 border-b border-white/60 py-8 md:py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            >
              <FaIcon icon={treatmentIcon(item.slug, item.body_area)} className="text-2xl" />
            </div>
            <span className={`badge border capitalize ${bodyAreaStyle(item.body_area)}`}>
              {item.body_area}
            </span>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-12 flex-1 w-full space-y-6">
        ...
        {(phases.length > 0 || item.treatment_plan) && (
          <div className="glass-card">
            <h2 className="font-semibold text-lg mb-4 flex items-center gap-2 text-primary-700">
        ...
        <div className="glass-strong rounded-2xl p-6 md:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2 shrink-0">
            <Link to={bookTreatmentUrl(item)} className="btn-primary text-center inline-flex items-center justify-center gap-2">
        ...
        {related.length > 0 && (
          <div>
            <h3 className="font-semibold text-slate-800 mb-4">Related {item.body_area} treatments</h3>
            <div className="grid sm:grid-cols-3 gap-4">
              {related.map((r) => (
                <Link
                  key={r.id}
                  to={`/treatments/${r.slug}`}
                  className="glass-card p-4 block hover:border-primary-300/50"
                >
                  <span className={`badge text-[10px] capitalize ${bodyAreaStyle(r.body_area)}`}>
                    {r.body_area}
                  </span>
