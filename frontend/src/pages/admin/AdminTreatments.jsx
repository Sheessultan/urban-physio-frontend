...
import { TREATMENT_BODY_AREAS } from '../../utils/treatmentHelpers';
...
export default function AdminTreatments() {
  ...
  const openEdit = async (id) => {
    ...
    setEditingId(id);
    try {
      const res = await admin.treatmentGet(id);
      const c = res.data;
      setForm({
        title: c.title || '',
        slug: c.slug || '',
        body_area: c.body_area || 'general',
      ...
      toast.error(err.message || 'Could not load treatment');
      setModalOpen(false);
      setEditingId(null);
    } finally {
      setLoadingEdit(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await admin.treatmentUpdate(editingId, payload);
        toast.success('Treatment updated');
      } else {
        await admin.treatmentCreate(payload);
  ...
  const handleDeactivate = async (id, title) => {
    ...
    try {
      await admin.treatmentDelete(id);
  ...
  const handlePermanentDelete = async (id, title) => {
    ...
    try {
      await admin.treatmentDelete(id, true);
      toast.success('Treatment deleted permanently');
      load();
    } catch (err) {
      toast.error(err.message);
    }
  ...
  const toggleActive = async (item) => {
    try {
      await admin.treatmentUpdate(item.id, { is_active: item.is_active ? 0 : 1 });
  ...
  return (
    <AdminDashboardLayout>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Manage Treatments</h1>
          <p className="text-slate-600 text-sm mt-1">Add, edit, or remove treatment pages shown on the website</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card overflow-x-auto p-0">
            ...
            <tbody>
              {list.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 hover:bg-white/30">
                  ...
                  <td className="p-4 capitalize">{c.body_area}</td>
          ...
          {!list.length && (
            <p className="text-center text-slate-500 py-12">No treatments found. Add your first one.</p>
          )}
        </div>
      )}
